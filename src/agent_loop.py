...
- Filed money facts the user wants always-on ("pin that money fact", "keep this in memory", "pin that") → `manage_memory` action=pin with memory_id from list/search. unpin removes always-on context. Pin does not delete.
...
- "List/show/analyze my todo list" / "what's on my todo list" / "categorize my todos" → call `manage_notes` with `action=list` FIRST. User todos and checklists live in Odysseus Notes — NOT in Notion, NOT in `manage_tasks`. `manage_tasks` is the scheduled background-jobs panel (recurring AI automations); an empty `manage_tasks` list does NOT mean the user has no todos. **Never call Notion unless the user explicitly mentions Notion or a Notion page/database.**
...
- "List/show/analyze my todo list" / "what's on my todo list" / "categorize my todos" / "agent vs human" triage → call `manage_notes` with `action=list` FIRST. User todos and checklists live in Odysseus Notes — NOT in Notion, NOT in `manage_tasks`. `manage_tasks` is the scheduled background-jobs panel; an empty `manage_tasks` list does NOT mean the user has no todos. **Never call Notion unless the user explicitly mentions Notion or a Notion page/database.** For handoffs to Cursor/Claude, use `create_document` with title `handoff → {target}: {title}` and YAML frontmatter (`source`, `target`, `status: pending`, `project`). For job applications, include a `## Full job description` section with the parsed JD verbatim — Odysseus auto-syncs it to `job-application-ops/positions/_active/<folder>/JD.md`. Tell the receiving agent to read that local `JD.md`; do not web-search Handshake.
...
- "Open/show <panel>" (documents, library, gallery, email, inbox, sessions, brain/memories, skills, settings, notes, cookbook, cmd-center) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, images→gallery, mail/inbox/emails→email, chats/history→sessions, memory/memories→brain, preferences→settings, models/serve/serving→cookbook, vault/command center/cmd center→cmd-center. CRITICAL: "open memory/memories/brain" / "open skills" / "open notes" / "open documents" / "open cookbook" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool. The "manage_*" tools list contents in chat; `ui_control open_panel` opens the visual modal the user is asking for.
- DISABLED (Docker): do NOT call `app_api` POST `/api/clicky/start` — the Linux container cannot spawn the Windows WPF overlay. For "start/launch/run Clicky" tell the user to run `deploy/scripts/start-clicky.ps1` on the Windows host (repo root). Clicky is the cursor-overlay app — NOT a note, task, or model.
...
- Filed money facts: `manage_memory` action=pin with memory_id from list/search. Pin does not delete.
"""
...
- User identity facts/preferences ("my name is X", "call me X", "I live in X") use `manage_memory`, not contacts.
- Filed money facts: `manage_memory` action=pin with memory_id from list/search. Pin does not delete.
- "Open/show <panel>" (documents, library, gallery, email, inbox, sessions, brain/memories, skills, settings, notes, cookbook, cmd-center) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, images→gallery, mail/inbox/emails→email, chats/history→sessions, memory/memories→brain, preferences→settings, models/serve/serving→cookbook, vault/command center/cmd center→cmd-center. CRITICAL: "open memory/memories/brain" / "open skills" / "open notes" / "open documents" / "open cookbook" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool.
- DISABLED (Docker): do NOT call `app_api` POST `/api/clicky/start` — the Linux container cannot spawn the Windows WPF overlay. For "start/launch/run Clicky" tell the user to run `deploy/scripts/start-clicky.ps1` on the Windows host (repo root). Clicky is the cursor-overlay app — NOT a note, task, or model.
"""
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_memory": "- ```manage_memory``` — Manage the user's persistent memory (facts, identity, preferences, money facts, context that persists across chats). Line 1 = action (list/add/edit/delete/search/pin/unpin), rest = content. pin/unpin need memory_id from list or search ('pin that money fact', 'keep this in memory'). Pin does not delete — it flags the fact for always-on Jarvis context (same as POST /api/memory/{id}/pin). Use when user says 'remember this', states identity facts like 'my name is <name>' / 'call me <name>' / 'I live in <place>', or asks about stored memories.",
    ...
    "manage_tasks": "- ```manage_tasks``` — Scheduled BACKGROUND AI JOBS (Tasks panel), NOT the user's todo list. For todos/checklists use `manage_notes` action=list. Args (JSON): {\"action\": \"list|create|edit|delete|pause|resume|run\", ...}",
...
{"action": "list"}
```
Notes, checklists, AND user reminders — the canonical todo source. Call `action=list` FIRST when the user asks to see/analyze/categorize todos. Use `add` for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours"). Actions: `list`, `add`, `update`, `delete`, `toggle_item`. Never use Notion for todos unless the user explicitly says their tasks are in Notion.""",
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "ui_control": "- ```ui_control``` — Control the UI: toggle tools on/off, OPEN PANELS, open email reply drafts, switch models, change themes. Commands: `toggle <name> on/off` (names: bash/shell, web/search, research, incognito, document_editor/documents), `open_panel <name>` (panels: documents, gallery, email, sessions, notes, memories/brain, skills, settings, cookbook, cmd-center), `open_email_reply <uid> <folder> <reply|reply-all|ai-reply>` (opens an email compose document, does NOT send), `set_mode agent/chat`, `switch_model <name>`, `set_theme <preset>`, `create_theme <name> <bg> <fg> <panel> <border> <accent>` (optional key=val for advanced colors AND background effects: bgPattern=<none|dots|synapse|rain|constellations|perlin-flow|petals|sparkles|embers>, bgEffectColor=#RRGGBB, bgEffectIntensity=<num>, bgEffectSize=<num>, frosted=true|false). \"open documents\" / \"open library\" / \"show gallery\" / \"open inbox\" / \"open notes\" / \"open cookbook\" / \"open vault\" / \"open cmd center\" all map to `open_panel <name>` (vault/command center/cmd center→cmd-center). Built-in theme presets: dark, light, midnight, paper, cyberpunk, retrowave, forest, ocean, ume, copper, terminal, organs, lavender, gpt, claude, cute. For any other vibe/name, use create_theme.",
...
- DISABLED (Docker): Clicky launch via POST `/api/clicky/start` — container cannot spawn Windows WPF; host script `deploy/scripts/start-clicky.ps1`. Status GET `/api/clicky/status` still exists but returns `supported: false` in Docker.
...
def _build_system_prompt(
    ...
    if not suppress_local_context:
        ...
        try:
            _memory_on = True
            try:
                from routes.prefs_routes import _load_for_user as _load_mem_prefs
                _memory_on = (_load_mem_prefs(owner) or {}).get("memory_enabled", True)
            except Exception:
                pass
            if _memory_on:
                from src.memory import load_pinned_memory_facts
                _pinned_facts = load_pinned_memory_facts(owner)
                if _pinned_facts:
                    _pinned_text = "\n- ".join(_pinned_facts)
                    _pinned_memory_message = untrusted_context_message(
                        "saved memory: pinned user facts",
                        f"Core facts about the user:\n- {_pinned_text}",
                    )
        except Exception as _pin_err:
            logger.debug("pinned memory injection skipped: %s", _pin_err)
    ...
    if _pinned_memory_message:
        merged.insert(last_user_idx, _pinned_memory_message)
        last_user_idx += 1
    if _skills_message:
...
async def stream_agent_loop(
    ...
    owner: Optional[str] = None,
    relevant_tools: Optional[Set[str]] = None,
    strict_relevant_tools: bool = False,
    ...
    if strict_relevant_tools and _relevant_tools:
        logger.info("[tool-rag] strict_relevant_tools=True — skipping domain expansion")
    ...
    # times out.
    if not guide_only and not strict_relevant_tools and _relevant_tools is not None:
