...
class AITTSManager {
    ...
    async play(text) {
        ...
        try {
            ...
            await applySinkToMediaElement(this.currentAudio);
            if (epoch !== this._playEpoch) return;
            ...
            if (this.useBrowserTTS) {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    applySinkToMediaElement(audio).then(() => {
                        ...
                        _emitFirstAudioOut('server');
                        this.isPlaying = true;
                        // Simulated viz only — createMediaElementSource after setSinkId
                        // can mute Chromium playback and leave the button "playing" with silence.
                        emitVoiceSpeaking(true);
                    }).catch(reject);
                });
            }
