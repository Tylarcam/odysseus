...
**Windows:** prefer the cmd launcher (System32 `bash` is WSL and breaks `pipefail`):

```bat
cmd /c "<SKILL_DIR>\scripts\run.cmd" "<TOPIC>" --days 30
```

Or call the engine directly: `bun <SKILL_DIR>/scripts/index.ts "<TOPIC>" --days 30`.
If you use bash, use Git Bash (`"C:\Program Files\Git\usr\bin\bash.exe"`), not WSL.
